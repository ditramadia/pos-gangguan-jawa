package com.example.if2210_tb2_nge.components;

import com.example.if2210_tb2_nge.entity.Items;
import com.example.if2210_tb2_nge.repository.ItemsRepository;
import javafx.geometry.Insets;
import javafx.geometry.Pos;
import javafx.scene.control.Button;
import javafx.scene.control.Label;
import javafx.scene.control.TextField;
import javafx.scene.image.Image;
import javafx.scene.image.ImageView;
import javafx.scene.input.KeyEvent;
import javafx.scene.layout.HBox;
import javafx.scene.layout.VBox;
import javafx.scene.text.Font;
import lombok.Getter;

import java.util.List;

import static org.apache.commons.lang.StringUtils.isNumeric;

public class MenuCard {
    @Getter
    private VBox cardContainer;
    private String imageUrl;
    private Image image;
    private ImageView imageView;
    @Getter
    private Label itemName;
    private HBox quantityContainer;
    @Getter
    private TextField quantity;
    private Button quantityIn;
    private Button quantityDe;
    private Label price;
    private int number;
    @Getter
    private int currentID;

    public MenuCard (Integer i) {
        cardContainer = new VBox();cardContainer.setStyle("-fx-background-color: #D7CDC7; -fx-background-radius: 20");
        cardContainer.setPrefWidth(250);
        cardContainer.setPrefHeight(250);
        cardContainer.setStyle("-fx-background-color: #D7CDC7; -fx-background-radius: 8");

        imageUrl = "data:image/jpeg;base64,";

        itemName = new Label("");
        Font fontItem = Font.loadFont("file:src/assets/Montserrat-Bold.ttf", 25);
        itemName.setFont(fontItem);
        itemName.setStyle("-fx-text-fill: #8C7466 ;");

        quantity = new TextField();
        Font fontQuantity = Font.loadFont("file:src/assets/Montserrat-Bold.ttf", 15);
        quantity.setFont(fontQuantity);
        quantity.setText("0");
        quantity.setPrefWidth(40);
        quantity.setAlignment(Pos.CENTER);
        quantity.setStyle("-fx-background-color: #D7CDC7; -fx-text-fill: #8C7466;");
        quantity.addEventFilter(KeyEvent.KEY_TYPED, event -> {
            String input = event.getCharacter();
            if (!isNumeric(input)) {
                event.consume();
            }
        });

        Font fontQ = Font.loadFont("file:src/assets/Montserrat-Bold.ttf", 15);

        quantityIn = new Button("+");
        quantityIn.setFont(fontQ);
        quantityIn.setPrefSize(30, 30);
        quantityIn.setStyle("-fx-background-color: #D7CDC7; -fx-text-fill: #8C7466; -fx-border-radius: 8px; -fx-border-color: #8C7466; -fx-border-width: 2px;");
        quantityIn.setOnAction(event -> {
            number++;
            quantity.setText(Integer.toString(number));
        });

        quantityDe = new Button("-");
        quantityDe.setFont(fontQ);
        quantityDe.setPrefSize(30, 30);
        quantityDe.setStyle("-fx-background-color: #D7CDC7; -fx-text-fill: #8C7466; -fx-border-radius: 8px; -fx-border-color: #8C7466; -fx-border-width: 2px;");
        quantityDe.setOnAction(event -> {
            number--;
            quantity.setText(Integer.toString(number));
        });

        quantityContainer = new HBox(10, quantityDe, quantity, quantityIn);
        HBox.setMargin(quantityIn, new Insets(5, 0, 0, 0));
        HBox.setMargin(quantityDe, new Insets(5, 0, 0, 0));
        HBox.setMargin(quantity, new Insets(5, 0, 0, 0));
        quantityContainer.setAlignment(Pos.CENTER);

        price = new Label("");
        this.loadData(i);
        cardContainer.getChildren().addAll(quantityContainer, price);

    }

    public void loadData(Integer i) {

        currentID = i;

        List<Items> items = ItemsRepository.getItems();
        imageUrl = "data:image/jpeg;base64,";
        for (Items item : items) {
            if (item.getId() == i){
                // item name
                if (item.getImage().length() >  5){
                    imageUrl = item.getImage();
                }
                itemName.setText(item.getName());
                price.setText("Rp " + item.getPrice().toString());
                Font fontPrice = Font.loadFont("file:src/assets/Montserrat-Bold.ttf", 20);
                price.setFont(fontPrice);
                price.setStyle("-fx-text-fill: #8C7466;");
                cardContainer.setAlignment(Pos.TOP_CENTER);
                VBox.setMargin(itemName, new Insets(10,0,10,0));
            }
        }

        // image
        image = new Image(imageUrl);
        imageView = new ImageView(image);
        imageView.setFitHeight(130);
        imageView.setFitWidth(130);
        imageView.setStyle("-fx-background-radius: 8;");
        cardContainer.getChildren().add(itemName);
        cardContainer.getChildren().add(imageView);
    }
}
